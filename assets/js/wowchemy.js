...
document.querySelectorAll('pre > code').forEach((codeblock) => {
  ...
  const copyIcn = document.createElement('i');
  const copyTxt = document.createElement('span');
  let classesToAdd = ['btn', 'btn-outline-primary', 'my-1', 'btn-copy-code'];
  let classesIconToAdd = ['fas', 'fa-copy']
  copyBtn.classList.add(...classesToAdd);
  copyIcn.classList.add(...classesIconToAdd);
  copyTxt.innerHTML = " " + i18n['copy'];
  copyBtn.appendChild(copyIcn);
  copyBtn.appendChild(copyTxt);

  function copiedNotification() {
    copyBtn.lastElementChild.innerHTML = " " + i18n['copied'];
    setTimeout(() => {
      copyBtn.lastElementChild.innerHTML = " " + i18n['copy'];
    }, 2000);
  }
